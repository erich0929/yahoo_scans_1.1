...
long get_stock_from_yahoo (char* code, 
		...
		char* to_mm, char* to_dd, char* to_yy, GPtrArray* empty_data) {
		
		empty_data = g_ptr_array_new ();
		GPtrArray* fulled_data = empty_data;
	...
	/*	int output = open ("output2.csv", O_WRONLY | O_CREAT | O_TRUNC, 0755);
		...
		int t = 1;
		while (str_to_array (line, fulled_data)) {
